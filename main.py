...
# Can check first if it is outside outline, if so answer is known without checking cut
...
def data_extract(filename, row_count):
    ...
    shape_type = temp_row[0]       # un used at the moment but could be used for optimisations mentioned at the top
    ...
    for j in range(1, num_points):
        temp_row = raw_data[row_count + j + 1].split(" ")  # [j+1] required to skip initial 'Outline/cut X' row
...
# finds equation form y=mx + c between given points p1,p2
def line_eq(px1, py1, px2, py2):
    ...
    # shouldn't have any divide by 0 issues as vertical lines should not lead to a mid_intersection
